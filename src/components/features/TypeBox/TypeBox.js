...
const TypeBox = ({
  ...
  const [timeAnimationShow, setTimeAnimationShow] = useState(false);
  const [timeAnimationValue, setTimeAnimationValue] = useState(0);
  const [timeAnimationType, setTimeAnimationType] = useState('bonus');
  ...
  const UpdateInput = (e) => {
    ...
    inputWordsHistory[currWordIndex] = newValue.trim();
    setInputWordsHistory(inputWordsHistory);
    
    // Update currChar based on actual input for mobile compatibility
    const trimmedValue = newValue.trimStart();
    if (trimmedValue.length > 0 && currCharIndex >= 0) {
      const actualChar = trimmedValue[currCharIndex];
      if (actualChar) {
  ...
  const showTimeAnimation = (value, type) => {
    setTimeAnimationValue(value);
    setTimeAnimationType(type);
    setTimeAnimationShow(true);
    setTimeout(() => setTimeAnimationShow(false), 1000);
  };
    ...
    if (status === "finished" && wpm > 0 && onAddToLeaderboard && !hasAddedToLeaderboard.current) {
      ...
      if (checkAndUnlockBadges) {
        const currentStats = {
          ...
          timerDuration: countDownConstant,
  ...
  return (
    <>
      <TimeAnimation 
        show={timeAnimationShow} 
        value={timeAnimationValue} 
        type={timeAnimationType} 
      />
